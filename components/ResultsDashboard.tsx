...
export default function ResultsDashboard({ result }: { result?: SimulationResult }) {
  ...
  const cards = [['Final value', brl(result.finalValue)], ['Total return', pct(result.totalReturn)], ['CAGR', pct(result.cagr)], ['Dividends received', brl(result.totalDividends)], ['Dividends reinvested', brl(result.totalDividendsReinvested)], ['Annual dividend income', brl(result.annualDividendIncome)], ['Cash balance', brl(result.cashBalance)], ['Best / worst', `${result.best?.ticker ?? '-'} / ${result.worst?.ticker ?? '-'}`]];
