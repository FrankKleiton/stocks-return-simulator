...
export default function ResultsDashboard({ result }: { result?: SimulationResult }) {
  ...
  const cards = [['Final value', brl(result.finalValue)], ['Total return', pct(result.totalReturn)], ['Money-weighted return', pct(result.moneyWeightedAnnualizedReturn)], ['Time-weighted return', pct(result.timeWeightedAnnualizedReturn)], ['Dividends received', brl(result.totalDividends)], ['Dividends deployed', brl(result.totalDividendsReinvested)], ['Annual dividend income', brl(result.annualDividendIncome)], ['Cash balance', brl(result.cashBalance)], ['Best / worst', `${result.best?.ticker ?? '-'} / ${result.worst?.ticker ?? '-'}`]];
