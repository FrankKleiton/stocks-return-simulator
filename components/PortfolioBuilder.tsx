...
export default function PortfolioBuilder({ items, setItems }: { items: PortfolioItem[]; setItems: (x: PortfolioItem[]) => void }) {
  ...
  return <Card withBorder shadow="md" radius="lg" p={{ base: 'sm', sm: 'md' }}>
    <Stack gap="md">
      ...
      <form action={addManual}><Group gap="xs" wrap="nowrap"><TextInput name="ticker" placeholder="Adicionar ação, ex: WEGE3" style={{ flex: 1 }}/><Button type="submit">Adicionar</Button></Group></form>
      <Stack gap="xs">{items.map((it, idx) => <Group key={it.ticker} gap="xs" wrap="nowrap" p="sm" className="portfolio-stock-row"><Badge size="lg" radius="sm" variant="gradient" gradient={{ from: 'cyber.5', to: 'plasma.5', deg: 135 }} miw={76}>{it.ticker}</Badge><NumberInput min={0} max={100} value={it.allocation} onChange={v => setItems(items.map((x,i)=>i===idx?{...x,allocation:Number(v) || 0}:x))} suffix="%" style={{ flex: 1 }}/><Button variant="light" color="red" size="xs" onClick={() => setItems(items.filter(x=>x.ticker!==it.ticker))}>Remover</Button></Group>)}{!items.length && <Text size="sm" c="dimmed">Adicione ações pelas recomendações ou busque manualmente.</Text>}</Stack>
    </Stack>
  </Card>;
}
